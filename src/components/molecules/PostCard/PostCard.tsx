...
function PostCardComponent({
  ...
  return (
    <article className={`bg-white dark:bg-dark-card overflow-hidden mb-3 relative ${
      isPrayer ? 'border border-primary-500/30' : ''
    }`}>
      {/* Badge de petición */}
      {isPrayer && (
        <div className="absolute top-3 left-16 z-20 flex items-center gap-2 bg-gradient-to-r from-primary-500/80 to-purple-600/80 px-3 py-1.5 rounded-full" style={{ top: '12px' }}>
          <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
            <path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd" />
          </svg>
          <span className="text-xs font-semibold text-white">Petición de Oración</span>
        </div>
      )}
      ...
      {/* Botón de Intercesión - Solo para peticiones activas */}
      {isPrayer && (
        <div className="px-3 pb-3 pt-2">
          <div className="flex items-center justify-between gap-3 p-3 bg-gray-100 dark:bg-dark-hover rounded-xl border border-primary-500/20 dark:border-primary-500/20">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-gray-900 dark:text-white font-medium text-sm">
                  {localIntercessionsCount > 0 
                    ? `${localIntercessionsCount} ${localIntercessionsCount === 1 ? 'hermano ha orado' : 'hermanos han orado'}`
                    : 'Sé el primero en interceder'
                  }
                </p>
              </div>
            </div>
            <button
              onClick={handleIntercedeClick}
              disabled={localHasInterceded || isInterceding}
              className={`px-4 py-2 rounded-lg font-semibold text-sm transition-all ${
                localHasInterceded
                  ? 'bg-green-500/20 text-green-400 border border-green-500/30 cursor-not-allowed'
                  : isInterceding
                  ? 'bg-gradient-to-r from-primary-500 to-purple-600 text-white'
                  : 'bg-gradient-to-r from-primary-500 to-purple-600 hover:from-primary-600 hover:to-purple-700 text-white hover:scale-105 active:scale-95'
              }`}
            >
              {localHasInterceded ? (
