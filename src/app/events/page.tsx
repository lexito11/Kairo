...
const CHRISTIAN_DENOMINATION_CATEGORIES = [
  'Pentecostal',
  'Bautista',
  'Evangélica',
  'No denominacional',
...
export default function EventsPage() {
  ...
  const getFilteredEvents = (): EventData[] => {
    ...
    // Filtro por categorías según el scope
    if (eventScope === 'cristianos' || eventScope === 'iglesia') {
      if (selectedChristianCategories.length > 0) {
        events = events.filter((event) =>
          selectedChristianCategories.includes(event.denomination)
        )
      }
      if (selectedChristianTypes.length > 0) {
        events = events.filter((event) => {
          const cat = event.category.toLowerCase()
          return selectedChristianTypes.some((type) => {
            const t = type.toLowerCase()
            if (t.startsWith('cultos')) return cat.includes('culto')
            if (t.startsWith('estudios')) return cat.includes('estudio')
            if (t.startsWith('conferencias')) return cat.includes('conferencia')
            if (t.startsWith('retiros')) return cat.includes('retiro')
            if (t.startsWith('alabanza')) return cat.includes('alabanza')
            if (t.startsWith('bautismos')) return cat.includes('bautism')
            return false
          })
        })
      }
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                          isSelected
                            ...
                            : 'bg-dark-hover text-gray-300 hover:text-white hover:bg-dark-border'
                        }`}
                      >
                        {tipo}
                      </button>
